...
class GeminiStructuredClaimClient:
    ...
    def _prompt(self, candidate_texts: tuple[str, ...], page: int) -> str:
        ...
        return (
            ...
            "Claim으로 만들지 않습니다.\n"
            "- `[표 문맥: ...]`은 PDF 표에서 분리된 상위 행·열 제목을 파서가 "
            "같은 수치 행에 다시 붙인 것입니다. 괄호 안의 Scope·산정방식·"
            "지역·연도 순서를 해당 행의 명시적 문맥으로 사용합니다.\n"
            "- 표 문맥에 `추출 대상`이 명시되어 있으면 같은 행의 다른 "
            "수치가 아니라 지정된 대상만 Claim으로 만듭니다.\n"
            "- 온실가스 총배출량·직접배출량·간접배출량 실적의 metric은 "
            "언어와 표의 행 이름에 관계없이 반드시 '온실가스 배출량'으로 "
            "기록합니다. 감축량·원단위·배출집약도·BAU·배출원별 값은 이 "
            "표준 지표명으로 바꾸지 않습니다.\n"
