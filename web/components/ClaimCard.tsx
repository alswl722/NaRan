...
import { formatDateOnly, formatDateRange, formatValueWithUnit } from "@/lib/format";
    ...
    <div className="rounded-2xl border border-line bg-surface p-5 shadow-card">
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <div className="mt-1.5 flex flex-wrap gap-x-3 gap-y-0.5 text-[12px] text-faint">
            ...
            {claim.period_start && claim.period_end && (
              <span>{formatDateRange(claim.period_start, claim.period_end)}</span>
            )}
          </div>
        </div>
      ...
      <div className="mt-2 flex flex-wrap items-center gap-x-2 gap-y-1 text-[11.5px] text-faint">
        ...
        <span className="rounded-full bg-faint/10 px-2 py-0.5 font-medium text-muted">
        ...
        <div className="mt-4 flex flex-col gap-4">
          {comparisons.map((comp, i) => (
            <div key={i} className="border-t border-line pt-4">
              ...
              <p className="mt-2 text-[13px] leading-relaxed text-ink">{comp.verdict.explanation}</p>
              ...
              {comp.verdict.absolute_difference !== null && (
                <div className="mt-2 flex flex-wrap gap-x-5 gap-y-1 text-[13.5px] tabular-nums text-ink-strong">
                  ...
                  {comp.verdict.relative_difference_pct !== null && (
                    <span>
                      상대 차이율{" "}
                      <strong>{comp.verdict.relative_difference_pct}%</strong>
                    </span>
                  )}
                </div>
              )}
              ...
              {comp.public_fact && (
                <div className="mt-3 rounded-xl bg-bg px-3.5 py-2.5 text-[12px] leading-relaxed text-muted">
                  <div>
                    공개 데이터{" "}
                    <span className="font-medium text-ink">
                      {formatValueWithUnit(comp.public_fact.raw_value, comp.public_fact.unit)}
                    </span>
                  </div>
                  <div className="mt-0.5 flex flex-wrap gap-x-3 text-faint">
                    <a
              ...
              {comp.verdict.review_required && comp.verdict.review_reasons.length > 0 && (
                <div className="mt-2 text-[12px] text-status-possible">
                  검토 필요 · {comp.verdict.review_reasons.join(" · ")}
                </div>
              )}
            </div>
          ))}
